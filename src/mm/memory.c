...
{
    uint32_t *prevPageDir = memGetCurrentPageDir();

    // if (virutalAddr >= KERNEL_START){
    //     prevPageDir = memGetCurrentPageDir(); // get current directory
    ...
    {
        uint32_t ptPAddr = pmmAllocPageFrame(); // if it isn't present create a new frame

        pageDir[pdIndex] = ptPAddr | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE | PAGE_FLAG_OWNER | flags; // set the page dir to adress
